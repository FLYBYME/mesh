import type { NodeInfo } from '../types/registry.schema.js';
...
export class KademliaRoutingTable {
    ...
    findNodesForTool(toolName: string, count: number): NodeInfo[] {
        const results: NodeInfo[] = [];
        for (const bucket of this.buckets) {
            for (const node of bucket) {
                if (node.services.some(s => s.name === toolName || (s.tools && s.tools[toolName]))) {
                    results.push(node);
                }
