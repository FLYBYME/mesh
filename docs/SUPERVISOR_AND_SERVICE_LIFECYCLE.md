...
**Open question carried into Part 3 — now resolved, see below:** a whole `MeshNetwork`/`MeshApp` process is pinned to exactly *one* namespace value for its entire life (set once at construction, e.g. via `TestAppOptions.namespace`). There was no way for a single process's `ServiceBroker` to host two different-namespace instances of the *same* domain simultaneously — `localTools` was keyed by `${domain}.${action}` with no namespace component, so a second `registerModule` call for an already-mounted domain would collide, not coexist.

**Resolution (`mesh` commit `76f0bb2`):** rather than route around this with a second broker/process per test run, the root collision was fixed directly. `ServiceBroker.registerModule(module, { key? })` now takes an optional mount key (defaults to the module's own `domain` — fully backward compatible, byte-for-byte the same behavior when omitted). A module mounted under a different key becomes a second, fully independent local instance: every contract it owns (including a *secondary* domain the same module owns, like `demo`'s own `demometrics` split — a real regression caught and fixed before this landed) is aliased through a new `effectiveToolDomain` helper so two mounted instances can never collide on any tool key. The aliased instance is never advertised to the Registry (local-only by construction — can't collide with the real instance's Registry entry or be routed to remotely), and `unregisterModule` now operates on mount key + instance identity rather than domain equality, so removing one instance can never disturb another sharing the same real domain. See `ServiceBroker.spec.ts`'s `describe('registerModule() — mount keys / aliased instances', ...)` for the load-bearing proof. This means Part 3 *can* run an isolated `test`-mount-keyed instance of a service inside the same Supervisor process as the real one, with the Supervisor's own manifest/control surface needing only to grow an optional mount-key concept when it's built — no further framework-level blocker remains.
...
### Part 3 — status: built (`mesh` commit `4dfb091`)

Built substantially as specced, with two real deviations from the original text above, both found while building and resolved deliberately rather than silently:

1. **Isolation mechanism is mount keys + per-mount databases, not literal `namespace`.** Step 2's own work found that cross-*node* isolation by `namespace` already existed for real at the packet layer (`MeshNetwork` drops any inbound packet whose namespace differs from the receiver's) — but that's node-level, and doesn't help a *single* Supervisor process that wants to run an isolated test instance of a domain already running in that same process. That gap turned out to need two separate framework additions, both built as their own commits before Part 3 itself: `ServiceBroker.registerModule`'s new `key` option (a mount-keyed instance is a second, fully independent local instance — its own tools, never advertised to the Registry, can't collide with the real one) and its new `database` option (routes that mount's own CRUD/time-series calls to a dedicated `Database` instead of the broker's shared default). Together these are the real load-bearing isolation Part 3 needed — `namespace` itself turned out not to be the right lever for *this* (same-process) case, even though making it real was still worthwhile in its own right (step 2).
2. **Addressing is by manifest `name`, not `domain`.** `supervisor.run_tests({ name, testName? })`, not `{ domain, testName? }` as sketched above — consistent with every other `supervisor.*` contract (`service_start`/`stop`/`restart`/`status` all address by manifest `name`), and necessary once two entries can share one real `domain` via `mountKey`.

**Test-association convention** (per the instruction above to pick one, not leave it as an open menu): a manifest entry's optional `testsPath` points at a sibling module exporting `tests: Record<string, (ctx: SupervisorTestContext) => Promise<void>>` — a test passes by resolving, fails by throwing, using a plain `assert` (normal-looking test functions, no custom framework). `SupervisorTestContext` carries `{ broker, mountKey }` — `mountKey` is the entry's actual registered key (its `domain`, unless the entry set its own `mountKey`), so a test file can build tool keys generically without knowing whether it's running against the real instance or an aliased test one.

`Supervisor.runTests(name, testName?)` finds the entry's *currently-running* instance (real, live, mounted via Part 1/2 — not a fresh throwaway one, exactly as specced), dynamically imports `testsPath`, runs the requested test (or every one, in declaration order), never lets one test's throw abort the run, and returns `{ passed, failed, results: [{ name, ok, error? }] }`. Real, honest errors for the edge cases: no `testsPath` configured, a named test that doesn't exist, or the entry not currently running — none of these silently report zero-results.

**Recommended usage shape**, given the resolution above: a manifest declares two entries pointing at the same `path` — one production (no `mountKey`/`database`, the default shared broker/DB) and one `-test`-suffixed entry with its own `mountKey` and `database`, plus `testsPath`. `run_tests` is pointed at the test entry's `name`. `run_tests` itself doesn't enforce this shape (it'll happily run tests against a production entry with no isolation if the manifest is written that way) — isolation is the manifest author's decision, not something the Supervisor decides on its own.

Verified: new fixtures `widget.service.ts` (a real CRUD-backed module) and `widget.tests.ts` (one passing test, one deliberately failing one) in `src/__tests__/fixtures/supervisor/`. `Supervisor.spec.ts` runs both a production and a mount-key+database-isolated entry pointed at the same fixture, calls `runTests` directly and through the real `supervisor.run_tests` mesh contract, and — the real proof — inspects the raw MongoDB collections directly afterward to confirm the test run's writes landed only in the isolated database, never in the production one. Also fixed two bugs surfaced while wiring this end-to-end: `DatabaseMiddleware`'s action-name extraction assumed a contract's real domain is always an exact prefix of the tool key (broke for an aliased key like `test:widget.create`), and `MeshApp.registerModule` silently dropped any `options` argument (only ever declared one parameter) — both fixed in the mount-key/database-routing commits, see `git log` for detail. Full suite: 268/275 (7 pre-existing unrelated `AuthInterceptor.spec.ts` failures, unchanged; confirmed stable across repeated full-suite runs, including ruling out an unrelated pre-existing `TimeSeries.spec.ts` flake — reproduced identically on the commit before any of this session's work).
...
1. `ServiceBroker.unregisterModule` (Part 1) — the real load-bearing test above must pass before anything else is built on top of it. **Built**, `f76236b`.
2. `namespace`-aware routing in `Registry` (a real prerequisite for Part 3, cheap to do now while touching this area). **Built**, `5ca6e13`. Turned out not to be the actual isolation mechanism Part 3 ended up using (see Part 3's status note) — still real, correct, and worth having for cross-node isolation in its own right.
3. The Supervisor itself (Part 2) — config parsing, dependency-ordered start, dynamic `import()`, the control surface (pick CLI-only vs. contract-based vs. both, per the open question above, before starting this). **Built**, `3435ba7` — control surface resolved as both, per the doc's own recommendation.
4. Test-running (Part 3) — needs 1-3 already real. **Built**, `4dfb091`, on top of two additional framework commits it turned out to need first: mount keys (`76f0bb2`) and per-mount database routing (`e53d5dd`) — see Part 3's status note for why `namespace` alone (step 2) wasn't sufficient.

All four steps complete as of 2026-08-25. Everything in this doc is built, tested (including real multi-node/real-MongoDB integration tests, not just unit tests against mocks), and pushed to `origin/master`.
